...
//	}
	...
	//根据三级分类的ID查询商品，带分页
	public PageBean<Product> findByPageTCid(Integer thirdCategoryId, int page) throws Exception {
		PageBean<Product> pageBean = new PageBean<Product>();
		//设置页码
		pageBean.setPage(page);
		//设置每页显示记录数
		...
		pageBean.setLimit(limit);
		//设置总记录数
		int totalCount = productDao.findTotalCountByTCid(thirdCategoryId);
		pageBean.setTotalCount(totalCount);
		//设置总页数
		int totalPage = 0;
		if(totalCount % limit == 0) {
		...
		//开始的地方
		int begin = (page - 1) * limit;
		List<Product> list = productDao.findByPageTCid(thirdCategoryId, begin, limit);
		pageBean.setList(list);
		return pageBean;
	}
